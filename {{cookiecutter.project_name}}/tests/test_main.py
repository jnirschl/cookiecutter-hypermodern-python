...
class TestMain:
    ...
    def test_main_dry_run(self, runner: CliRunner, temp_dir: str) -> None:
        """Test dry run."""
        result = runner.invoke(__main__.main, [temp_dir, "--dry-run", "--debug"])
        assert result.exit_code == 0

    def test_main_success(self, runner: CliRunner, temp_dir: str) -> None:
        """Test successful run."""
        result = runner.invoke(__main__.main, [temp_dir, "--debug"])
        assert result.exit_code == 0

    def test_main_version(self, runner: CliRunner, temp_dir: str) -> None:
        """Test version."""
        result = runner.invoke(__main__.main, [temp_dir, "--version", "--debug"])
        assert result.exit_code == 0
        assert (
    ...
    def test_invalid_input_dir(self, runner: CliRunner, invalid_dir: str) -> None:
        ...
        result = runner.invoke(__main__.main, [invalid_dir, "--debug"])
